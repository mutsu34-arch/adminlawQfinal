...
const FALLBACK_MODELS = ["gemini-1.5-flash", "gemini-2.5-flash", "gemini-2.5-flash-lite"];
  ...
  return db().runTransaction(async (t) => {
    ...
    if (count >= QUIZ_AI_DAILY_LIMIT) {
      throw new HttpsError(
        "resource-exhausted",
        `오늘 AI 질문 한도(${QUIZ_AI_DAILY_LIMIT}회)를 모두 사용했습니다. 내일 다시 이용해 주세요.`
      );
    }
    const next = count + 1;
    t.set(
...
const quizAskGemini = onCall({ region: "asia-northeast3" }, async (request) => {
  ...
  if (!quiz.statement || quiz.statement.length < 3) {
    throw new HttpsError("invalid-argument", "문항 정보가 없습니다. 퀴즈 화면에서 다시 시도해 주세요.");
  }
  ...
  try {
    const answer = await generateQuizReply(apiKey, modelId, quiz, rawQ, ragContext, learnerNick);
    return {
      ok: true,
      answer,
